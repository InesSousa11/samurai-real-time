...
def run_sequence(
    ...
    for fidx, fp in enumerate(frames):
        ...
        for gid, bb in gt_bb_by_id_all.items():
            ...
            draw_text_with_bg(vis, f"GT {gid}", (bb[0], max(15, bb[1] - 6)), fg=color, bg=(0, 0, 0))

        for det_idx, bb in enumerate(det_bboxes):
            info = det_best_match.get(det_idx, {})
            txt = f"Y {info.get('best_id','?')} {float(info.get('best_sim', -1.0)):.2f}"
            cv2.rectangle(vis, (bb[0], bb[1]), (bb[2], bb[3]), (80, 80, 80), 1)
            draw_text_with_bg(vis, txt, (bb[0], min(height - 5, bb[3] + 15)), fg=(180, 180, 180), bg=(0, 0, 0), scale=0.45)

        # Orange boxes: suspicious predictions. They remain visible even if ignored.
        for pred_id, bb in pred_bbox_by_id_raw.items():
            if int(pred_id) not in suspicious_pred_ids:
                continue
            color = (0, 165, 255)
            cv2.rectangle(vis, (bb[0], bb[1]), (bb[2], bb[3]), color, 2)
            draw_text_with_bg(vis, f"IGN PR {pred_id}", (bb[0], bb[1] + 18), fg=color, bg=(0, 0, 0), scale=0.55)

            if save_review_frames:
                reason = suspicious_pred_reason.get(int(pred_id), "unknown")
        ...
        for pred_id, bb in pred_bbox_by_id_eval.items():
            ...
            cv2.rectangle(vis, (bb[0], bb[1]), (bb[2], bb[3]), color, 2)
            draw_text_with_bg(vis, f"PR {pred_id}", (bb[0], bb[1] + 18), fg=color, bg=(0, 0, 0), scale=0.6)
