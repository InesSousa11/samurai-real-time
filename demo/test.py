...
# ============================================================
# DEBUG: emulate ROS2 wrapper behavior by feeding BGR to model
# ============================================================
USE_BGR_FOR_MODEL = True  # True = send BGR to predictor, False = keep RGB

def to_model_frame(rgb_frame: np.ndarray) -> np.ndarray:
    """Convert UI RGB frame to the color format the model will receive."""
    if rgb_frame is None:
        return None
    return cv2.cvtColor(rgb_frame, cv2.COLOR_RGB2BGR) if USE_BGR_FOR_MODEL else rgb_frame
...
def process_frame(rgb_frame):
    if rgb_frame is None:
        return None
    state["last_frame"] = rgb_frame  # keep RGB for UI
    ...
    if state["tracking"] and not state.get("injecting", False):
        try:
            # feed model in chosen color space
            model_frame = to_model_frame(rgb_frame)

            out_obj_ids, out_mask_logits = predictor.track(model_frame)
...
def on_accept():
    ...
    if not state["tracking"]:
        if not state["first_frame_loaded"]:
            predictor.load_first_frame(to_model_frame(state["last_frame"]))
    ...
    try:
        ...
        predictor.add_conditioning_frame(to_model_frame(state["last_frame"]))
