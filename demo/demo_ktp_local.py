# demo_ktp_local.py
# Local (non-Colab) version of your KTP Gradio demo.
# Keeps the same logic, but:
#  - removes Google Drive mount
#  - makes repo/ckpt/cfg/ktp_root configurable via env vars or defaults
#  - ensures sam2 import works by adding REPO to sys.path (or PYTHONPATH)
#  - makes CUDA knobs safe (only enable if cuda is available)
#
# Run:
#   python demo_ktp_local.py
#
# Optional env vars:
#   SAMURAI_REPO="C:\path\to\samurai-real-time"   (or /home/.../samurai-real-time)
#   SAM2_CKPT="C:\...\checkpoints\sam2.1_hiera_small.pt"
#   SAM2_CFG="configs\samurai\sam2.1_hiera_s.yaml"   (relative to repo) or absolute path
#   KTP_ROOT="C:\path\to\KTP"  (contains images/ and ground_truth/)
#   FACE_YOLO_CKPT="C:\...\checkpoints\yolov8n-face.pt" (optional)
...
# 1) Repo root (where sam2/ and configs/ are)
REPO = os.environ.get("SAMURAI_REPO", os.path.abspath(os.path.dirname(__file__)))
# If you put this script inside the repo (recommended), REPO will be the script directory.
# If not, set SAMURAI_REPO env var.

# Add repo to python path so "import sam2" works
if REPO not in sys.path:
    sys.path.insert(0, REPO)

# 2) Checkpoint + config
DEFAULT_CKPT = os.path.join(REPO, "checkpoints", "sam2.1_hiera_small.pt")
DEFAULT_CFG_REL = os.path.join("configs", "samurai", "sam2.1_hiera_s.yaml")

CKPT = os.environ.get("SAM2_CKPT", DEFAULT_CKPT)
CFG  = os.environ.get("SAM2_CFG", DEFAULT_CFG_REL)

# Make CFG absolute if user passed relative
if not os.path.isabs(CFG):
    CFG = os.path.join(REPO, CFG)

# Face YOLO optional
DEFAULT_FACE_YOLO = os.path.join(REPO, "checkpoints", "yolov8n-face.pt")
YOLO_FACE_CKPT = os.environ.get("FACE_YOLO_CKPT", DEFAULT_FACE_YOLO)

# Default KTP path (edit this to your local dataset)
DEFAULT_ROOT = os.environ.get("KTP_ROOT", os.path.join(REPO, "thesis_datasets", "KTP"))

# =========================================================
# Performance knobs (CUDA-safe)
# =========================================================
...
try:
    ...
    raise ModuleNotFoundError(
        ...
        f"REPO used: {REPO}\n"
        "Fix:\n"
        "  1) Set SAMURAI_REPO to the samurai-real-time repo root\n"
        "  2) Or place this script inside the repo\n"
        "  3) Or export PYTHONPATH to include the repo root\n"
        "Example (bash):\n"
        "  export SAMURAI_REPO=/path/to/samurai-real-time\n"
        "  export PYTHONPATH=$SAMURAI_REPO:$PYTHONPATH\n"
        "  python demo_ktp_local.py\n"
    ) from e
...
print(f"[sam2] Loaded predictor with:\n  CFG={CFG}\n  CKPT={CKPT}")
...
def _export_csv(obj_id:int):
    ...
    path = os.path.join(out_dir, f"scores_obj_{int(obj_id)}.csv")
...
def _export_summary_csv():
    out_dir = os.path.join(os.getcwd(), "outputs")
    os.makedirs(out_dir, exist_ok=True)
    outp = os.path.join(out_dir, "ktp_per_frame_assign.csv")
...
def run_ktp_sequence(
    ...
    if save_overlay_video:
        ...
        out_dir = os.path.join(os.getcwd(), "outputs")
        os.makedirs(out_dir, exist_ok=True)
        video_path = os.path.join(out_dir, f"{seqname}_overlay.mp4")
