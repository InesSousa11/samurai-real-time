...
RealSense debug harness to "see inside" the SAMURAI/SAM2 pipeline.
...
Run example:
  python .\demo\realsense_deep_debug_reid.py --reid_thr 0.80 --reid_print

Optional:
  python .\demo\realsense_deep_debug_reid.py --width 1280 --height 720 --fps 15
  python .\demo\realsense_deep_debug_reid.py --serial <YOUR_CAMERA_SERIAL>
"""

print("=== realsense_deep_debug_reid.py VERSION: INTERNAL-REID-004 (MODEL-OWNED-REID) ===", flush=True)
...
def create_realsense_pipeline(width: int, height: int, fps: int, serial: Optional[str] = None):
    ...
    try:
        ...
        serial_out = "UnknownSerial"

    return pipeline, profile, name, serial_out


def get_display_size(win_name: str, fallback_w: int, fallback_h: int, cache: dict):
    """
    Read current OpenCV window image area size, but ignore obviously bad tiny values.
    Keeps the last good size in `cache`.
    """
    ww = int(cache.get("w", fallback_w))
    wh = int(cache.get("h", fallback_h))
    ...
    try:
        ...
        # Ignore bogus tiny readings from some OpenCV backends
        if cur_w >= 320 and cur_h >= 240:
            ww, wh = cur_w, cur_h
            cache["w"] = ww
            cache["h"] = wh
    except Exception:
...
def render_for_window(img: np.ndarray, target_w: int, target_h: int, mode: str = "fit"):
    """
    Return an image whose final size is exactly (target_w, target_h).

    mode="fit"     -> preserve aspect ratio, pad with gray borders if needed
    mode="stretch" -> fill the whole window exactly, may distort aspect ratio
    """
...
def main():
    ...
    ap.add_argument("--display_width", type=int, default=1400)
    ap.add_argument("--display_height", type=int, default=900)
    ap.add_argument("--display_mode", type=str, default="fit", choices=["fit", "stretch"])

    ap.add_argument("--yolo_conf", type=float, default=0.25)
    ap.add_argument("--out_root", type=str, default=str(REPO_ROOT2 / "debug_cases_realsense"))
    ...
    CFG_FILE_CHECK = REPO_ROOT2 / "sam2" / "configs" / "samurai" / "sam2.1_hiera_s.yaml"

    if not CKPT_PATH.exists():
        raise FileNotFoundError(f"Checkpoint not found:\n  {CKPT_PATH}")
    if not CFG_FILE_CHECK.exists():
    ...
    predictor = build_sam2_camera_predictor(CFG_NAME, str(CKPT_PATH), reid_backend_name=args.reid_backend)

    print(f"[init] ReID backend: {args.reid_backend}", flush=True)
    ...
    print(
        f"[init] Starting RealSense color stream "
        f"({args.width}x{args.height} @ {args.fps} FPS)...",
        flush=True,
    )
    pipeline, rs_profile, rs_name, rs_serial = create_realsense_pipeline(
        width=args.width,
        height=args.height,
        fps=args.fps,
        serial=args.serial,
    )
    print(f"[init] RealSense device: {rs_name} | serial={rs_serial}", flush=True)
    ...
    def _ring_store(global_fidx: int, rgb_img: np.ndarray):
        if noncond_keys.maxlen is not None and len(noncond_keys) >= noncond_keys.maxlen:
            old = noncond_keys[0]
    ...
    try:
        while True:
            try:
                ...
                if not color_frame:
                    print("[rs] No color frame.", flush=True)
                    continue
                bgr = np.asanyarray(color_frame.get_data())
            except Exception as e:
                print(f"[rs] Frame grab failed: {repr(e)}", flush=True)
                break
